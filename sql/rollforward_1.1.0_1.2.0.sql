...
#-- System      : SenDb
#--
#-- Filename    : rollforward_1.1.0_1.2.0.sql
#--
...
migrate:BEGIN
    ...
    IF (versionTableExists = 0) THEN
        SELECT 'This script can only run on a version 1.1.0 database' AS result
        UNION
    ...
    END IF;
                                                            ...
                                                            #-----------------------------------------------------------
    ALTER TABLE events
    CHANGE COLUMN startdate startdatetime DATETIME NOT NULL;

    ALTER TABLE events
    CHANGE COLUMN enddate enddatetime DATETIME NOT NULL;

    UPDATE events
    SET
        enddatetime = DATE_ADD(enddatetime, INTERVAL 1 DAY);
