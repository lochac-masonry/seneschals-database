...
#-- System      : SenDb
#--
#-- Filename    : rollforward_2.0.0_2.1.0.sql
#--
...
migrate:BEGIN
    ...
    IF (versionTableExists = 0) THEN
        SELECT 'This script can only run on a version 2.0.0 database' AS result
        UNION
    ...
    END IF;
                                                            ...
                                                            #-----------------------------------------------------------
    ALTER TABLE events
    ADD COLUMN notifyInsurer BOOLEAN NOT NULL AFTER description;
    ...
    SELECT 'Rollforward complete. Now at version 2.1.0.' AS result;
