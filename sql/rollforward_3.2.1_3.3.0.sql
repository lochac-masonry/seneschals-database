...
#-- System      : SenDb
#--
#-- Filename    : rollforward_3.2.1_3.3.0.sql
#--
...
migrate:BEGIN
    ...
    IF (versionTableExists = 0) THEN
        SELECT 'This script can only run on a version 3.2.1 database' AS result
        UNION
    ...
    ALTER TABLE events
    RENAME COLUMN notifyInsurer TO meetsInsuranceConditionHighIncome,
    ADD COLUMN meetsInsuranceConditionAnimalUse BOOLEAN NOT NULL AFTER meetsInsuranceConditionHighIncome;

                                                            #-----------------------------------------------------------
                                                            #-- Log deployment
                                                            #-----------------------------------------------------------
    INSERT INTO version (version) VALUES ('3.3.0');

    SELECT 'Rollforward complete. Now at version 3.3.0.' AS result;
