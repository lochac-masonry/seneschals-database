...
class PostcodeController extends Zend_Controller_Action
{
    ...
    public function queryAction()
    {
        ...
        $form->setAction('#')
             ->setMethod('post')
             ->addElement('checkbox','printable',array('label' => 'Printable Report?'))
             ->addElement('select','group',array('label' => 'Group name:', 'multiOptions' => $groupList))
             ->addElement('submit','querybygroup',array('label' => 'Submit'))
             ->addDisplayGroup(array('group', 'querybygroup'), 'byGroup', array('legend' => 'Search by Group'))
             ->addElement('text','postcode',array('label' => 'Postcode:',
                                                  'validators' => array(array('stringLength',false,array(0, 4)))))
             ->addElement('submit','querybycode',array('label' => 'Submit'))
             ->addDisplayGroup(array('postcode', 'querybycode'), 'byCode', array('legend' => 'Search by Postcode'))
             ->addElement('text','rangestart',array('label' => 'Range Start:',
                                                  'validators' => array(array('stringLength',false,array(0, 4)))))
             ->addElement('text','rangeend',array('label' => 'Range End:',
                                                  'validators' => array(array('stringLength',false,array(0, 4)))))
             ->addElement('submit','querybyrange',array('label' => 'Submit'))
             ->addDisplayGroup(array('rangestart', 'rangeend', 'querybyrange'), 'byRange', array('legend' => 'Search by Postcode Range'))
             ->addElement('text','locality',array('label' => 'Suburb/Locality Name:',
                                                  'validators' => array(array('stringLength',false,array(0,64)))))
        ...
        if($form->isValid($_POST)) {
            ...
            if($queryExists) {
                $db->setFetchMode(Zend_Db::FETCH_OBJ);
                try { $results = $db->fetchAll($sql); }
                catch (Exception $e) { $this->view->message .= "<div class='bad'>Possible error fetching data.</div><br />\n"; }
                foreach ($results as $result) {
                    ...
                    foreach ($localities[$result->postcode] as $locality) {
                        ...
                        else $result->localities .= ', ' . $locality;
                    }
                }
    ...
    public function assignAction()
    {
        ...
        $form->setAction('#')
             ->setMethod('post')
             ->addElement('text','rangestart',array('label' => 'Postcode Range Start:',
                                                    'required' => true,
                                                    'filters' => array('stringTrim'),
                                                    'validators' => array('int', array('stringLength',false,array(0,4)))))
             ->addElement('text','rangeend',array('label' => 'Range End:',
                                                  'required' => true,
                                                  'filters' => array('stringTrim'),
                                                  'validators' => array('int', array('stringLength',false,array(0,4)))))
             ->addElement('select','group',array('label' => 'Assign to:',
        ...
        if($form->isValid($_POST)) {
            ...
            if (move_uploaded_file($userfile['tmp_name'], $targetfile)) {
                ...
                while(!feof($file)) {
                    ...
                    foreach($rowData as $key => $value) $row[$headRow[$key]] = $value;
                    ...
                    if($exists === 0) {
                        ...
                        try { $db->insert('postcode',array('current' => 'Y',
                                                     ...
                                                     'state' => $row['State'],
                                                     'groupid' => $groupID, ));
                        } catch(Exception $e) { $this->view->message .= "<div class='bad'>Possible error adding $row[Locality], " .
                                                                        "$row[Pcode], $row[State].</div><br />\n";
                        }
                        $insertCount++;
                    } else {
                        // Update with current details.
                        try { $db->update('postcode',array('current' => 'Y',
                                                     'comments' => $row['Comments'],
                                                     'deliveryoffice' => $row['DeliveryOffice'],
                                                     'presortindicator' => $row['PresortIndicator'],
                                                     'parcelzone' => $row['ParcelZone'],
                                                     ...
                                                     'bspname' => $row['BSPname'],
                                                     'category' => $row['Category'], ),
                                               array("postcode={$db->quote($row['Pcode'],Zend_Db::INT_TYPE)}",
                                                     "locality={$db->quote($row['Locality'])}",
                                                     "state={$db->quote($row['State'])}", ));
                        } catch(Exception $e) { $this->view->message .= "<div class='bad'>Possible error updating $row[Locality], " .
                                                                        "$row[Pcode], $row[State].</div><br />\n";
                        }
                        $updateCount++;
                    }
                }
                try { $deleteCount = $db->delete('postcode',"current='N'");
                } catch(Exception $e) { $this->view->message .= "<div class='bad'>Possible error deleting old entries.</div><br />\n";
                }
